		...
		var v = $("#contactform").validate({
			submitHandler: function (form) {
				...
				Email.send({
					...
					Username: "anjumantech@gmail.com",
					Password: "762D80B0F1D9974D169354545E0A5DB329C7",
					To: 'info@qcservices.com.au',
					From: 'anjumantech@gmail.com',
					Subject: "Someone Messaged you through website : " + subject,
					Body: "Name: <b>" + name + '</b><br>' + "email: <b>" + email + "</b><br>" + "Subject: <b>" + subject + "</b><br>" + "Message: <b>" + message + "</b><br>"
				}).then(
					$('#contactform #email').val(''),
					$('#contactform #name').val(''),
					$('#contactform #subject').val(''),
					$('#contactform #message').val(''),
					$('#contactform #result').text('Message send Successfully. Someone from our team will get back to you soon'),
					$('#contactform #result').delay(8000).fadeOut('slow')
					//message => console.log(message)
					// message => alert("Message send Successfully. Someone from our team will get back to you soon")
				);
...
$(window).on('load', function () {
	...
	// make active link visual like buttton (26-06-24)

	$('.nav-link').on('click', function () {
		console.log("Filter clicked");
		$('.nav-link').removeClass('makeButton');
		$(this).addClass('makeButton');
	});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Initialize Viewer on document load
    initViewer();

    // Reinitialize Viewer on carousel change
    $('.dtr-slick-slider').on('afterChange', function(event, slick, currentSlide){
      if (viewer) {
        viewer.update();
      } else {
        initViewer();
